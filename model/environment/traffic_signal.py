...
class TrafficSignal:
    ...
    def __init__(
        ...
        self.action_space = spaces.MultiDiscrete(
            [self.num_green_phases, self.max_green - self.min_green + 1]
        )
    ...
    def set_next_phase(self, action: List[int]):
        """Set the next green phase and duration for the traffic signal.

        Args:
            action: List containing phase (int) and duration (int).
        """
        self.action = action
        new_phase = action[0]
        green_duration = (
            action[1] + self.min_green
        )  # Get the green duration from the input

        if (
            self.green_phase == new_phase
            or self.time_since_last_phase_change < self.yellow_time + self.min_green
        ):
            # Stay in the current green phase
            self.sumo.trafficlight.setRedYellowGreenState(
                self.id, self.all_phases[self.green_phase].state
            )
            self.next_action_time = (
                self.env.sim_step + green_duration
            )  # Update next_action_time based on green_duration
        else:
            # Transition to the yellow phase
            self.sumo.trafficlight.setRedYellowGreenState(
                self.id,
                self.all_phases[self.yellow_dict[(self.green_phase, new_phase)]].state,
            )
            self.green_phase = new_phase
            self.green_duration = green_duration  # Use the input green duration
            self.next_action_time = (
                self.env.sim_step + self.yellow_time
            )  # Update next_action_time based on yellow_time
            self.is_yellow = True
            self.time_since_last_phase_change = 0
    ...
    def get_lanes_max_count(self) -> List[int]:
        """Returns the maximum number of vehicles that could fit in the incoming lanes of the intersection."""
        lanes_max_count = [
            self.lanes_length[lane]
            / (self.MIN_GAP + self.sumo.lane.getLastStepLength(lane))
            for lane in self.lanes
        ]
        return [min(1, density) for density in lanes_max_count]
    ...
    def get_halting_lanes_count(self) -> List[int]:
        """Returns the number of vehicles halting in the incoming lanes of the intersection."""
        return [self.sumo.lane.getLastStepHaltingNumber(lane) for lane in self.lanes]

    def get_lanes_count(self) -> List[int]:
        """Returns the number of vehicles in the incoming lanes of the intersection."""
