...
class SumoEnvironment(gym.Env):
    ...
    def step(self, action: Union[dict, Tuple[int, int]]):
        ...
        dones = True if self.current_step >= self.max_steps else False
        ...
        cum_reward = (
            self.traffic_signals[self.ts_ids[0]].reward_fn(None, True)
            if truncated
            else 0
        )
        ...
        if self.single_agent:
            return (
                ...
                (rewards[self.ts_ids[0]] + cum_reward) / 2 if truncated else rewards[self.ts_ids[0]],
                terminated,
                truncated,
                info,
            )
    ...
    def _apply_actions(self, actions):
        ...
        Args:
            actions: If single-agent, actions is tuple (phase, duration)
    ...
    def action_spaces(self, ts_id: str) -> gym.spaces.MultiDiscrete:
        """Return the action space of a traffic signal.
        chnaged to return MULTIDISCRETE(int(0-3),int(5-60))
        """
    ...
    def _get_per_agent_info(self):
        ...
        average_density = [
            sum(self.traffic_signals[ts].get_lanes_density())
            / len(self.traffic_signals[ts].get_lanes_density())
            for ts in self.ts_ids
        ]
        info = {}
        for i, ts in enumerate(self.ts_ids):
            ...
            info[f"{ts}_average_density"] = average_density[i]
        info["agents_total_stopped"] = sum(stopped)
        info["agents_total_accumulated_waiting_time"] = sum(accumulated_waiting_time)
        info["agents_average_density"] = sum(average_density) / len(average_density)
        return info
